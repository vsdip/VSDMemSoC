...
\SV
   ...
   m4_sv_get_url(m4_RISCV_SRAM_repo['testbench/bin/data.bin'])
\SV
   m4_sv_include_url(['https://raw.githubusercontent.com/MufuteeVC/RISC-V_Core_External_SRAM_IMem/main/sram_2_16_sky130A.v'])
...
\TLV
   ...
   *passed = |cpu/xreg[17]>>5$value == (1+2+3+4+5+6+7+8+9);
   ...
   |cpu 
      ...
      @1
         `BOGUS_USE($imem_rd_en)
         \SV_plus
            sram_2_16_sky130A imemory(
               *clk && !*reset,
               1'b0,
               1'b1,
               <<1$imem_rd_addr,
